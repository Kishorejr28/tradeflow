      ...
      <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl shadow-2xl w-full max-w-sm">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => setDirection('buy')} className={`py-3 rounded-xl text-sm font-bold transition ${direction === 'buy' ? 'bg-emerald-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-500'}`}>
              BUY {price.toFixed(decimals)}
            </button>
            <button onClick={() => setDirection('sell')} className={`py-3 rounded-xl text-sm font-bold transition ${direction === 'sell' ? 'bg-red-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-500'}`}>
          ...
          {/* Lots */}
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Volume (lots)</label>
            <div className="flex items-center gap-2">
              <button onClick={() => setLots(l => Math.max(0.01, parseFloat(l) - 0.01).toFixed(2))} className="w-9 h-9 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 font-bold hover:bg-gray-200 dark:hover:bg-gray-700 transition">−</button>
              <input value={lots} onChange={e => setLots(e.target.value)} className="flex-1 text-center py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500/30" />
              <button onClick={() => setLots(l => (parseFloat(l) + 0.01).toFixed(2))} className="w-9 h-9 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 font-bold hover:bg-gray-200 dark:hover:bg-gray-700 transition">+</button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Stop Loss</label>
              <input value={sl} onChange={e => setSl(e.target.value)} placeholder="Optional" className="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-400/30" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Take Profit</label>
              <input value={tp} onChange={e => setTp(e.target.value)} placeholder="Optional" className="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-400/30" />
            </div>
          </div>

          {/* Info */}
          <div className="text-xs text-gray-400 space-y-1 bg-gray-50 dark:bg-gray-800/50 rounded-lg p-3">
            <div className="flex justify-between"><span>Pip value</span><span className="text-gray-600 dark:text-gray-300">${pipVal.toFixed(2)} / pip</span></div>
            <div className="flex justify-between"><span>Est. margin</span><span className="text-gray-600 dark:text-gray-300">${margin.toFixed(0)}</span></div>
            <div className="flex justify-between"><span>Balance</span><span className="text-gray-600 dark:text-gray-300">${balance.toLocaleString()}</span></div>
          </div>

          <button onClick={handleOpen} className={`w-full py-3 rounded-xl text-white text-sm font-bold transition ${direction === 'buy' ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-red-500 hover:bg-red-600'}`}>
    ...
    <div className="flex h-full overflow-hidden">
      ...
      {showReset && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">This will close all positions and reset your balance to $100,000. This cannot be undone.</p>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center gap-3 px-4 py-2.5 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-[#141414] shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs font-mono font-bold ${direction[activeSymbol] === 'up' ? 'text-emerald-500' : 'text-red-500'}`}>
              {(prices[activeSymbol] ?? 0).toFixed(decimals(activeSymbol))}
            </span>
          </div>
          <div className="flex items-center gap-1 px-2 py-0.5 rounded bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400 text-[10px] font-semibold">
            DEMO
          </div>
          ...
          <button onClick={() => setShowOrder(true)} className="px-4 py-1.5 bg-brand-500 hover:bg-brand-600 text-white text-xs font-semibold rounded-lg transition">
            New Order
          </button>
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-[#141414] shrink-0" style={{ height: '160px' }}>
          <div className="flex items-center gap-4 px-4 py-1.5 border-b border-gray-100 dark:border-gray-800">
            {(['positions', 'history'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`text-xs font-medium pb-1 border-b-2 transition capitalize ${activeTab === tab ? 'border-brand-500 text-brand-600 dark:text-brand-400' : 'border-transparent text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}>
              ...
              positions.length === 0 ? (
                ...
                <table className="w-full text-xs">
                  <thead><tr className="text-gray-400 bg-gray-50 dark:bg-gray-900/50">
                    {['Symbol','Dir','Lots','Entry','Current','P&L','Pips','Action'].map(h => (
                    ...
                    {positions.map(pos => {
                      ...
                      return (
                        <tr key={pos.id} className="border-t border-gray-50 dark:border-gray-800/60 hover:bg-gray-50 dark:hover:bg-white/3">
                          ...
                          <td className="px-3 py-2 text-gray-600 dark:text-gray-300">{pos.entryPrice.toFixed(decimals(pos.symbol))}</td>
                          <td className="px-3 py-2 font-mono text-gray-800 dark:text-gray-200">{cur.toFixed(decimals(pos.symbol))}</td>
                          ...
                          <td className="px-3 py-2">
                            <button onClick={() => closePosition(pos.id, cur)} className="px-2 py-1 rounded bg-red-50 dark:bg-red-500/10 text-red-500 hover:bg-red-100 dark:hover:bg-red-500/20 font-medium transition">Close</button>
                          </td>
                        </tr>
                      )
                    })}
                  ...
                  <tbody>
                    {history.map(t => (
                      <tr key={t.id} className="border-t border-gray-50 dark:border-gray-800/60">
                        ...
                        <td className="px-3 py-2 text-gray-600 dark:text-gray-300">{t.entryPrice.toFixed(decimals(t.symbol))}</td>
                        <td className="px-3 py-2 text-gray-600 dark:text-gray-300">{t.exitPrice.toFixed(decimals(t.symbol))}</td>
      ...
      <div className="w-52 border-l border-gray-100 dark:border-gray-800 flex flex-col bg-white dark:bg-[#141414] shrink-0">
          ...
          {SYMBOLS.map(sym => (
            ...
            >
              <div>
                ...
                <p className={`text-[11px] font-mono mt-0.5 transition-colors ${direction[sym] === 'up' ? 'text-emerald-600' : 'text-red-500'}`}>
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 p-3">
          <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">Account</p>
          <div className="space-y-1.5 text-xs">
            <div className="flex justify-between text-gray-500 dark:text-gray-400">
              <span>Balance</span><span className="text-gray-700 dark:text-gray-300 font-medium">${balance.toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-gray-500 dark:text-gray-400">
              <span>Open P&L</span>
