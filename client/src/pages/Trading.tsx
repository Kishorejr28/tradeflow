...
import { TrendingUp, TrendingDown, BarChart2, X, RotateCcw, Star, ChevronUp, ChevronDown } from 'lucide-react'
...
// Floating starred symbols toolbar that sits above the chart
function StarredToolbar({ starred, prices, direction, activeSymbol, onSelect, onUnstar, decimals }: {
  starred: string[]
  prices: Record<string, number>
  direction: Record<string, 'up' | 'down'>
  activeSymbol: string
  onSelect: (s: string) => void
  onUnstar: (s: string) => void
  decimals: (s: string) => number
}) {
  if (starred.length === 0) return null
  return (
    <div className="flex items-center gap-1 px-3 py-1.5 bg-white dark:bg-[#1a1a1a] border-b border-gray-100 dark:border-gray-800 overflow-x-auto shrink-0">
      <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide mr-1 shrink-0">Starred</span>
      {starred.map(sym => {
        const price = prices[sym] ?? 0
        const up = direction[sym] === 'up'
        const active = sym === activeSymbol
        return (
          <div
            key={sym}
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg cursor-pointer group transition shrink-0 border ${
              active
                ? 'bg-brand-50 dark:bg-brand-500/15 border-brand-300 dark:border-brand-600'
                : 'hover:bg-gray-50 dark:hover:bg-white/5 border-transparent'
            }`}
            onClick={() => onSelect(sym)}
          >
            <span className="text-xs font-semibold text-gray-800 dark:text-gray-200">{sym}</span>
            <span className={`text-[11px] font-mono tabular-nums font-medium ${up ? 'text-emerald-500' : 'text-red-500'}`}>
              {price.toFixed(decimals(sym))}
            </span>
            {up ? <ChevronUp className="w-2.5 h-2.5 text-emerald-500" /> : <ChevronDown className="w-2.5 h-2.5 text-red-500" />}
            <button
              onClick={e => { e.stopPropagation(); onUnstar(sym) }}
              className="opacity-0 group-hover:opacity-100 text-gray-300 hover:text-amber-400 transition ml-0.5"
              title="Remove"
            >
...
export default function Trading() {
  ...
  const [starred, setStarred] = useState<string[]>(['EURUSD', 'XAUUSD'])

  const toggleStar = (sym: string) =>
    setStarred(p => p.includes(sym) ? p.filter(s => s !== sym) : [...p, sym])
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Starred toolbar — only shows if any symbols are starred */}
        <StarredToolbar
          starred={starred}
          prices={prices}
          direction={direction}
      ...
      <div className="w-52 border-l border-gray-100 dark:border-gray-800 flex flex-col bg-white dark:bg-[#141414] shrink-0">
        <div className="px-3 py-2.5 border-b border-gray-100 dark:border-gray-800">
          ...
          <p className="text-[10px] text-gray-400 mt-0.5">★ to pin to toolbar</p>
        </div>
        <div className="flex-1 overflow-y-auto">
          {SYMBOLS.map(sym => (
            <div key={sym}
              className={`w-full flex items-center justify-between px-3 py-2.5 transition border-l-2 group ${activeSymbol === sym ? 'border-brand-500 bg-brand-50 dark:bg-brand-500/10' : 'border-transparent hover:bg-gray-50 dark:hover:bg-white/5'}`}>
              <button className="flex-1 text-left" onClick={() => setActiveSymbol(sym)}>
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                <button
                  onClick={() => toggleStar(sym)}
                  className={`p-0.5 rounded transition ${starred.includes(sym) ? 'text-amber-400' : 'text-gray-300 dark:text-gray-600 opacity-0 group-hover:opacity-100 hover:text-amber-400'}`}
                  title={starred.includes(sym) ? 'Unstar' : 'Star'}
                >
